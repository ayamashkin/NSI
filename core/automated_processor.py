...
# 2026-06-05 10:45:00 — FIX: _fix_loaded_mask — Штифт: x между диаметром и длиной + составные покрытия.
...
class AutomatedParametricProcessor:
    ...
    def _fix_loaded_mask(self, mask):
        ...
        # FIX 5 2026-06-05 10:45 (МСК, UTC+3): Штифты — диаметр x длина + составные покрытия.
        # Проблема: Штифт 0,6х10 ГОСТ 3129-70 — между диаметром и длиной 'х', не [-\s]+.
        # В штифтах: \d+(?:,\d+)? — только запятая для диаметра
        pattern = pattern.replace(
            '(?P<наружный_диаметр_сторона_квадр>\\d+(?:,\\d+)?))(?:[-\\s]+(?P<длина>',
            '(?P<наружный_диаметр_сторона_квадр>\\d+(?:,\\d+)?))(?:[xXхХ×\\-\\s]+(?P<длина>'
        )
        # Проблема: хим.Окс.прм — марка_материала1 захватывает 'хим' перед точкой.
        pattern = pattern.replace(
