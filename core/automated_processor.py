...
# 2026-06-06 03:00:00 — FIX 11/12: ГОСТ 5915/3128/3129 полная замена маски.
#   5915: исполнение+М кирилл, шаг только через х, все params опциональны.
#   3128/3129: исполнение.диаметр опционально, покрытие опционально. 15/15 тестов.
# 2026-06-06 01:00:00 — FIX 10: ГОСТ 1491/17475 полная замена маски. SOFT_PARAMS={'длина'}.
#   ГОСТ 1491: 0%→100%, 17475: 0%→99%. +~2600 params.
...
class AutomatedParametricProcessor:
    ...
    def _apply_mask_fixes(pattern: str) -> str:
        ...
        if r'ГОСТ\s*1491' in pattern:
            pattern = (r'^Винт\s+(?P<класс_точности>[ABАВ])?[mм]'
