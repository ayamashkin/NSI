...
# 2026-06-08 12:25:00 — FIX 10v2: ГОСТ 1491/17475 шаг резьбы опционально. +~221 запись.
#   M10x1,25-100-40 = диам(10)+шаг(1,25)+дл(100)+покр(40). bugs2: 538/1875 (28.7%).
# 2026-06-08 12:20:00 — FIX 18/19: покрытие с дефисом, пробел после М. +~40.
# 2026-06-08 10:00:00 — FIX 14v3: суффикс после ОСТ/ГОСТ — (?:\s+.*)?$.
# 2026-06-08 8:00:00 — FIX 1b/13/14: Приоритет 1. +~1914 записей.
...
class AutomatedParametricProcessor:
    ...
    def _apply_mask_fixes(pattern: str) -> str:
        ...
        # Для "Хим.Пас.OCT" — точка между покрытием и ОСТ
        if '(?!P<покрытие>' in pattern:
            pattern = pattern.replace(
                r'(?P<покрытие>[\w.\-]+)[-\s]+OCT',
                r'(?P<покрытие>[\w.\-]+)[.\-\s]*OCT'
            )
