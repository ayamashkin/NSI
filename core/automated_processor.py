...
class AutomatedParametricProcessor:
    ...
    def _apply_mask_fixes(pattern: str) -> str:
        ...
        # FIX 10: Винт ГОСТ 1491/17475 — опциональный класс_допуска + х
        if pattern.startswith('^Винт') and (r'ГОСТ\s*1491' in pattern or r'ГОСТ\s*17475' in pattern):
            # Класс допуска + х опциональны: Винт 4-10-... (без класса)
            pattern = pattern.replace(
                r'[-\s]+(?P<класс_допуска>\d+[a-z])[-\s]+х(?P<длина>',
                r'(?:[-\s]+(?P<класс_допуска>\d+[a-z])[-\s]+х)?(?P<длина>'
            )
            # Если длина через дефис (не х): Винт 4-10-Кд-...
            pattern = pattern.replace(
                r'(?P<длина>\d+(?:[.,]\d+)?)(?:[-\s]+(?P<покрытие>[\w.]+))?[-\s]+ГОСТ',
                r'(?P<длина>\d+(?:[.,]\d+)?)(?:[-\s]+(?P<класс_допуска>\d+[a-z]))?(?:[-\s]+(?P<покрытие>[\w.]+))?[-\s]+ГОСТ'
            )
        return pattern
