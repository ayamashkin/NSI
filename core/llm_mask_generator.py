...
# 2026-05-29 11:26:00 — FIX: _extract_json_from_text handles \u003c escapes and (?P\( artifacts; _sanitize_mask_result fixes (?P\(
    ...
    def _build_meta_groups_rules(self, standard: str) -> str:
        ...
        for g in groups:
            if g == "тип_изделия":
                ...
                lines.append(
                    f"- \`{g}\` всегда добавляется в конец паттерна (стандарт: ГОСТ 7798-70, ОСТ 1 31133-80...)")
            else:
...
# =================================================="""
    ...
    def _extract_json_from_text(self, text: str) -> Optional[Dict]:
        ...
        # FIX 2026-05-29 11:26 UTC+3: replace Unicode escapes for angle brackets
        text = text.replace("\u003c", "<").replace("\u003e", ">")
        # FIX: replace escaped angle brackets in regex patterns
    ...
    def _fix_pattern(self, pattern: str, standard: str, item_type: str) -> str:
        ...
        # FIX 2026-05-29 11:42 UTC+3: handle LLM artifact (?P\( -> (?P<
    ...
    def _sanitize_mask_result(self, result: MaskGenerationResult) -> MaskGenerationResult:
        ...
        # FIX: normalize double-escaped regex sequences
        pattern = pattern.replace(r"\\d", r"\d").replace(r"\\s", r"\s").replace(r"\\w", r"\w")
        # FIX 2026-05-29 11:26 UTC+3: handle LLM artifact (?P\( -> (?P<
        pattern = pattern.replace("(?P\(", "(?P<")
