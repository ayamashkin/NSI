...
# 2026-05-27 21:15:00 — Added _relax_pattern for diagnose command compatibility
# 2026-05-27 21:15:00 — _relax_pattern replaces strict standard with flexible match
# 2026-05-22 08:50:00 — _get_matching_config reads from config.yaml matching section
# 2026-05-21 15:15:00 — fuzzy threshold 0.6->0.5, coating_substitution forces success
# 2026-05-21 08:50:00 — canonicalize_standard on extract & mask generation
# =============================================================================
...
LAST_FIXES:
 2026-05-27 21:15 UTC+3 — Added _relax_pattern for diagnose command.
...
class ParametricENSClient:
    ...
    def _relax_pattern(self, pattern: str, standard: str = "") -> str:
        """Relax regex pattern for diagnostic matching.

        Replaces strict standard group with flexible match to test
        pattern structure against arbitrary input.
        """
        if not pattern:
            return pattern
        relaxed = pattern
        # Replace нтд_1 group with flexible standard match
        if "(?P<нтд_1>" in relaxed:
            std_re = re.escape(standard) if standard else r"ОСТ|ГОСТ|ТУ"
            relaxed = re.sub(
                r"\(?P<нтд_1>[^)]+\)",
                f"(?P<нтд_1>{std_re}[^\\s]*)",
                relaxed
            )
        # Remove anchors for partial matching
